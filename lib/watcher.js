  ...
  */
  ...
  _trackPayments() {
    ...
    .then((transactions) => {
      ...
      const lastTx = transactions.records[transactions.records.length - 1];
      const opCount = lastTx.operations.length;
      this.cursor = new BigNumber(lastTx.paging_token).plus(opCount - 1).toString();
    ...
    this.closeStream = transactions
      ...
      .stream({
        ...
        onmessage: (rawTx) => {
          ...
          const opCount = rawTx.operations.length;
          this.cursor = new BigNumber(rawTx.paging_token).plus(opCount - 1).toString();
          this._enqueue([rawTx]);
        },
        onerror: (err) => {
